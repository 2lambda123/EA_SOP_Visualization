...
$.fn.evoAnimate = function (props) {
  ...
  var UNDER_CONTENT =
    ...
    POINT_CURRENT_COLOR +
    ';"></div> - Current generation <br/>' +
    '  	<div class="color-box" style="background-color:' +
    POINT_PREVIOUS_GEN_COLOR +
    ';"></div> - Parents <br/>' +
    '  	<div class="color-box" style="background-color:' +
    POINT_OLDER_COLORS +
    ';"></div> - Older <br/>' +
  ...
  function setProblemRange(data) {
    ...
    for (var i in data.steps) {
      var step = data.steps[i];
      if (TIMELINE_MIN_FITNESS < 0) step.actualFitness = step.fitness + TIMELINE_MIN_FITNESS * -1;
      else step.actualFitness = step.fitness;
    }
  ...
  function calculateShades(data, canvasObj) {
    ...
    if (max < numOfShades) {
      ...
      for (var i = max; i > 0; i--) shadeStarts[iterator--] = i;
    } else {
      var growth = false;
  ...
  function fillInfoTab(canvasObj) {
    ...
    var html = "" + d.algId + ": " + d.algName + "<br/>" + "Dimensions: " + d.problemDim + "<br/>";
    // Algorithm params
    for (var i = 0; i < d.algParams.length; i++) {
      if (i + 1 !== d.algParams.length) html += d.algParams + ", ";
      else html += d.algParams;
    }
    if (d.algParams.length) html += "<br/>";

    html += d.problemId + ": " + d.problemName + "<br/>";
    // Problem params
    for (var i = 0; i < d.problemParams.length; i++) {
      if (i + 1 !== d.problemParams.length) html += d.problemParams + ", ";
      else html += d.problemParams;
    }
    if (d.problemParams.length) html += "<br/>";
  ...
  function incrementShadeOnPoint(canvasObj, x, y, transformed = true) {
    ...
    var rtrn =
      (undefined === canvasObj.shadeStartsCounter[x]
        ? -1
        : undefined === canvasObj.shadeStartsCounter[x][y]
        ? -1
        : canvasObj.shadeStartsCounter[x][y]++) + 1;
    return rtrn;
  }
  ...
  function getShade(canvasObj, x, y) {
    ...
    if (counterValue > evolutionUtil.lastItem(shadeStarts)) return evolutionUtil.lastItem(CANVAS_SHADES_COLORS);
    if (counterValue > 0) return CANVAS_SHADES_COLORS[0]; //getShade hack
  ...
  function renderShades(canvasObj) {
    ...
    for (var i = 0; i < x; i++) {
      for (var j = 0; j < y; j++) {
        if (canvasObj.shadeStartsCounter[i][j] >= CANVAS_SHADES[canvasObj.id][0]) renderShadePoint(canvasObj, i, j);
      }
    }
  }
  ...
  function renderAllCanvasesShades() {
    for (var i in CANVAS_ARR) renderShades(CANVAS_ARR[i]);
  }
  ...
  function displaySearchedAreaInfo() {
    for (var i in CANVAS_ARR) drawSearchInfoOnCanvas(CANVAS_ARR[i]);
  }
  /*
  ...
  function parseInput(input) {
    ...
    evolutionUtil.indexOfAll(input, ["{", "}"], function (index, prev, count) {
      ...
      if ("" !== item.trim()) parseLine(rtrn, item);
    });
  ...
  function parseLineArg(obj, arg, argNum) {
    switch (argNum) {
      ...
      case 1: {
        ...
        if (0 === arg && !FIRST_GEN_IS_ZERO) FIRST_GEN_IS_ZERO = true;
        if (FIRST_GEN_IS_ZERO) arg++;
  ...
  function checkGenIsShown(genNum) {
    ...
    if (0 === SHOWN_GENERATIONS_NUMBER) return true;
    for (var i in RENDERED_GENERATIONS) if (RENDERED_GENERATIONS[i] === genNum) return true;
    return false;
  }
  /*
   * Finds  step by id
  ...
  function findStepById(id) {
    ...
    if (steps[id - 1].id === id) return steps[id - 1];
  ...
  function renderStep(x, y = 0, canvasObj, parentCoords, drawLine = true, lastStep = false) {
    ...
    // Increment shades
    if (isShadingHistory()) incrementShadeOnPoint(canvasObj, physicalCoords.x, physicalCoords.y);
  }
  ...
  function fadePoints(canvasObj, parents, childX, childY, childGenId) {
    if (!checkGenIsShown(childGenId - 1)) return;
    var ctx = canvasObj.renderLayerCtx;
    ...
    for (var i in parents) {
      ...
      if (checkGenIsShown(childGenId - 2)) {
        ...
        for (var i in parent.parentIds) {
          ...
          if (undefined === parentsParent) continue;
  ...
  function stepGen(data, genNumber) {
    ...
    var generationLength =
      (GENERATION_STARTS.length > genNumber ? GENERATION_STARTS[genNumber] : data.steps.length) - GENERATION_STARTS[genNumber - 1];
  ...
  */
  ...
  function setRenderedGenerations(num) {
    ...
    for (var i = num; i >= num - SHOWN_GENERATIONS_NUMBER + 1; i--) {
      if (i > 0) RENDERED_GENERATIONS.unshift(i);
    }
  ...
  var renderGenerations = function (lastGenId = -1, lastGenStepId = -1, drawLines = true) {
    ...
    if (lastGenStepId <= 0) {
      if (lastGenId < GENERATION_STARTS.length) lastGenStepId = GENERATION_STARTS[lastGenId];
      else lastGenStepId = ANIMATION_DATA.steps.length;
    }
  ...
  function checkRenderedGenerations(generationNum) {
    ...
    if (LAST_ADDED_GENERATION === generationNum) return;
  ...
  var play = function () {
    if (isLoaded() && !isPlaying()) {
      ...
      if (ANIMATION_DATA.steps.length > PLAY_STEP + 1) {
        PLAYBACK_STARTED_GEN =
          PLAYBACK_STARTED_GEN !== ANIMATION_DATA.steps[PLAY_STEP + 1].generation
            ? ANIMATION_DATA.steps[PLAY_STEP + 1].generation
            : PLAYBACK_STARTED_GEN;
      }
      // Play the animation
      animationLoop();
  ...
  var moveOneStepBackward = function () {
    ...
    if (PLAY_STEP > 0) {
      ...
      if (ANIMATION_DATA.steps.length === PLAY_STEP) PLAY_STEP--; // Subtract one more
      PLAY_STEP--;
      var stepData = ANIMATION_DATA.steps[PLAY_STEP];
  ...
  function calculateTimelineSteps(cWidth) {
    ...
    if (0 !== shownStepsNum % 2) {
      // If calulated number is odd, make it even, to get integers when dividing by 2
      shownStepsNum++;
    }
  ...
  function clearTimeline(canvasObj) {
    ...
    ctx.clearRect(
      0,
      canvasObj.height - TIMELINE_HEIGHT - TIMELINE_OFFSET_BOTTOM,
      canvasObj.width,
      TIMELINE_HEIGHT + TIMELINE_OFFSET_BOTTOM
    );
  }
  /*
  ...
  function menuButtonShow(ctx, show = false) {
    if (show === MENU_BUTTON_SHOWN) return;
    if (true === show) {
  ...
  function findPointsOnClick(offsetX, offsetY, canvasObj) {
    ...
    if (undefined === tmpLastStep) tmpLastStep = ANIMATION_DATA.steps.length;
    ...
    for (var i = startStep; i < lastStep; i++) {
      ...
      if (step.id > PLAY_STEP + 1) continue;
      // Check if step is still visible (it is not in an older generation)
      if (!checkGenIsShown(step.generation)) continue;
      var x = canvasObj.xIndex - 1;
      var y = canvasObj.yIndex - 1;
      var coords = coordinateTransform(canvasObj, step.x[x], step.x[y]);
      x = coords.x;
  ...
  function bindSettingsEvents(canvasObj) {
    ...
    $button.off("click").on("click", function () {
      ...
      if (meshLinesNumber > 100) meshLinesNumber = 100;
      if (meshLinesNumber < 0) meshLinesNumber = 10;
  ...
  function fillSettingsContainer($container) {
    $container.html(
      "" +
        "<table>" +
        "  <tbody>" +
        '    <tr><td>Iterations to jump:</td><td><input type="text" class="settings-generations-jump" value="' +
        JUMP_OVER_GENERATIONS_NUM +
        '" /></td></tr>' +
        '    <tr><td>Mesh lines number:</td><td><input type="text" class="settings-mesh-number" value="' +
        MESH_LINE_NUMBERS +
        '" /></td></tr>' +
  ...
  function bindEvents() {
    $.each(CANVAS_ARR, function (key, value) {
      ...
      $canvas.off("mousemove").on("mousemove", function (e) {
        ...
        if (checkMenuShown(canvasObj)) return;
      ...
      $canvas.off("mouseleave").on("mouseleave", function (e) {
    ...
    $(".evo-animate-canvas-container .evo-animate-under .button button")
      ...
      .on("click", function () {
        ...
        if ("none" === $content.css("display")) $content.slideDown();
  ...
  function initialize() {
    ...
    if (props.hasOwnProperty("display")) {
      ...
      if ($.isArray(display)) {
        ...
        if (pass && isArray) CANVAS_X_SETTING = display;
        else if (!isArray) CANVAS_X_SETTING = [display];
        else console.warn("All items within the display array must be arrays.");
      } else {
    ...
    if ("string" === sourceType) loadingCompleted();
